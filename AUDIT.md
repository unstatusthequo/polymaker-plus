...
3. **User websocket emitted misleading warnings**: A dangling `else` in `process_user_data` always logged "not in" even for valid tokens. The warning now fires only when a specific row references an unknown token.
